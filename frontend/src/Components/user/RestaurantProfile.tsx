...
const RestaurantProfile: React.FC = () => {
  ...
  const handleChatClick = async () => {
    ...
    try {
      ...
      let existingConversation = response.conversation.find(
        (conversation: any) =>
          conversation.members.includes(userId) && conversation.members.includes(restaurantId)
      );

      // If no existing conversation, create a new one
      if (!existingConversation) {
        existingConversation = await addConversation(userId, restaurantId);
      }

      // Navigate to the chat page with the conversation ID
      navigate('/chat', { state: { conversationId: existingConversation._id, openConversation:true } });
    } catch (error) {
      ...
      <div className="max-w-screen-lg mx-auto px-4">
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className='py-4'>
              <div className="flex flex-col mb-4">
                <div className='flex justify-between items-center'>
                  <h1 className="text-4xl font-bold">{restaurant.restaurantName}</h1>
                  <button 
                    className='bg-teal-600 text-white text-center p-2 rounded flex items-center'
                    onClick={handleChatClick}
                ...
                <div className="flex flex-col items-start mt-2">
                  <div className='flex items-center'>
                    <FaLocationDot className='text-teal-600 mr-1' />
                    <span className="text-gray-600">{restaurant.place}</span>
                  </div>
                  <div className='flex items-center'>
                    <FaMobileAlt className="text-teal-600 mr-1" />
                    <span className="text-gray-600">+91 {restaurant.contact}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:w-1/3 lg:sticky lg:top-20 space-y-6 pt-12">
            ...
            <div className="border p-4 rounded-lg shadow-md">
              <h2 className="text-2xl font-bold mb-4">Make a reservation</h2>
              <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); fetchAvailableTimeSlots(); }}>
                <div>
                  <label htmlFor="guestCount" className="block text-gray-700">Number of guests</label>
                  <select
                    id="guestCount"
                    value={guestCount}
                    onChange={(e) => setGuestCount(parseInt(e.target.value))}
              ...
              {fetchingTimeSlots ? (
                <p className="mt-4">Loading available slots...</p>
              ) : (
                availableTimeSlots.length > 0 ? (
                  <div className="mt-4">
                    <h3 className="text-md font-semibold mb-2">Available slots for the selected date</h3>
            ...
            <div>
              ...
              <div className="bg-gray-100 p-4 rounded-lg shadow-lg mb-6">
                ...
                <a 
                  ...
                  className="flex items-center space-x-4 mt-4 text-teal-600 hover:text-teal-800"
                >
                  <FaMapMarkerAlt />
